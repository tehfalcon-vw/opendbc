...
class SpeedLimitManager:
  ...
  def update(self, psd_06, psd_04):
    ...
    self._receive_speed_limit_fusion(mux, psd_06) # try reading speed from car camera + navigation fusion
    if psd_04:
      self._receive_street_type(psd_04)
      self._receive_speed_limit_legal(mux, psd_06) # try reading speed from legal limit by current street type
  ...
  def _receive_speed_limit_fusion(self, mux, psd_06):
    if mux == 2:
  ...
  def _receive_speed_limit_legal(self, mux, psd_06):
    if mux == 2:
