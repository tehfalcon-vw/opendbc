...
from opendbc.car import Bus, apply_driver_steer_torque_limits, apply_std_curvature_limits, structs
...
class CarController(CarControllerBase):
  ...
  def update(self, CC, CC_SP, CS, now_nanos):
      ...
      if self.CP.flags & VolkswagenFlags.MEB:
        ...
        if CC.latActive:
          ...
          if CS.out.steeringPressed: # roughly sync curvature when user overrides
            ...
            apply_curvature = np.clip(apply_curvature, -self.CCP.CURVATURE_LIMITS.CURVATURE_MAX, self.CCP.CURVATURE_LIMITS.CURVATURE_MAX)
    ...
    if self.CP.flags & VolkswagenFlags.MEB:
      ...
      self.blinker_timeout_counter = 0 if CS.out.leftBlinker or CS.out.rightBlinker or self.blinker_timeout_counter >= 10 else self.blinker_timeout_counter + 1
      if self.frame % 2 == 0:
        left_blinker = True if True and not CS.out.leftBlinker and self.blinker_timeout_counter >= 10 else False
        right_blinker = True if CC.rightBlinker and not CS.out.rightBlinker and self.blinker_timeout_counter >= 10 else False
