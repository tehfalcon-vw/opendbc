...
class CarController(CarControllerBase):
  ...
  def update(self, CC, CC_SP, CS, now_nanos):
    ...
    if self.CP.flags & VolkswagenFlags.MEB:
      ...
      self.blinker_timeout_counter = 0 if CS.out.leftBlinker or CS.out.rightBlinker or self.blinker_timeout_counter >= 2 else self.blinker_timeout_counter + 1
      if self.frame % 2 == 0:
        left_blinker = False #True if CC.leftBlinker and self.blinker_timeout_counter >= 2 else False
        right_blinker = False #True if CC.rightBlinker and self.blinker_timeout_counter >= 2 else False
